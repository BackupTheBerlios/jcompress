...
public class DemoTableBDD extends JFrame
{
	public DemoTableBDD()
	{
		...
		txtSQL.setText("SELECT * FROM Table1");
...
//        JPanel3.setLayout(new FlowLayout());
		...
		txtConnection.setText("BaseEssai1");
		txtConnection.setSelectionColor(new java.awt.Color(204,204,255));
		txtConnection.setSelectedTextColor(java.awt.Color.black);
		txtConnection.setCaretColor(java.awt.Color.black);
		txtConnection.setDisabledTextColor(new java.awt.Color(153,153,153));
		...
		txtConnection.setBounds(1,0,132,23);
		txtTable.setText("Table1");
		...
		JPanel4.add(JComboBox1,new GridBagConstraints(1,0,1,1,0.0,0.0,java.awt.GridBagConstraints.CENTER,java.awt.GridBagConstraints.NONE,new Insets(2,4,0,0),6,-5));
		JComboBox1.setBounds(137,2,137,19);
		...
		cmbBoxBDD.setBounds(453,2,90,20);
		{
			...
			tempString[0] = "BaseEssai1";
			tempString[1] = "sample-MySQL";
			tempString[2] = "Ludo";
    ...
    /**
     * export ds un fichier des lignes d une table d'une bdd
     * la 1ere ligne comportera les noms
     * la 2ime comportera les types
     * si pas de condition where toute la table est exporter
     * si pas de sep
     * le sep par defo est utilis?
     * on ajoute a la fin de la 1ere ligne le sep(et toute les otres ;o))
     */
...
//System.out.println( " SQL = " + sql );
          ...
          data.write( ligne ); data.newLine(); //ecri la ligne et passe a la suivante ...
            ligne = "";
            // 2eme ligne
            for ( int i = 1; i <= nbrCols; i++ )
...
System.out.println( "\n nombre d enreg lu = " + line );
            ...
            {
System.out.println( "SQL executer : " + sql );
...
//	        if ( base.length() > 0 )
	        {
                if ( connection != null ) connection.close();
                String nomBase = (String)this.cmbBoxBDD.getSelectedItem();
	            
	            
	            if ( nomBase.compareTo( "Oracle" ) == 0)// si on veu connecter a oracle
	            {
                    // connection a une base Oracle par jdbc
                    String url = "jdbc:oracle:thin:@URL:1521:BASE"; // evidement cette base n est ppas valide
                    Class.forName( "oracle.jdbc.driver.OracleDriver" );
                    connection = DriverManager.getConnection( url, "user", "pass" );
                }
                else if ( nomBase.compareTo( "MySQL" ) == 0)
                {
                    // connection a une base MySQL par jdbc
                    //jdbc:mysql://[hostname][:port]/dbname[?param1=value1][=value2]...
                    Class.forName("org.gjt.mm.mysql.Driver").newInstance();
                    String url   = "jdbc:mysql:///" + base;
                    connection = DriverManager.getConnection( url, "", "" );
                }
                else // ds tou les otre cas on ce conenct a ACCESS 
                {
...
System.out.println( " connectiuon effectuer a  :  " + mdt.getURL() );
            }
        }
        catch ( Exception e )
        {
...
//            tc.setCellRenderer( new LudoDemoCellRenderer() );
    ...
    {   // c juste pour montrer un exemple de coloriage de header ds le rendere
