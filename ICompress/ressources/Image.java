...
/**
 * Gestion d'une image en forme de matrice encapsulation de la matrice associee
 * import/export fichiers import/export arbre
 */
public class Image {
	...
	/**
	 * Construteur Suppose un fichier non vide et la taille annoncée est
	 * concordante avec le fichier sinon-->NullPointerException
	...
	private Noeud construireNoeud(Noeud p){
		...
		if(getMat() != null){
			if(getMat().getTaille() > 1){
				...
				return new Couleur(p, Integer.parseInt(getMat().get(0, 0)
						.getValeur()));
			}
		}
		return null;
	...
	/**
	 * fonction recursive de construction descendante des noeuds compresse sans
	 * perte
	 * @param p, noeud courant qui est le pere des prochains
	 * @return Noeud
	 */
	...
	/**
	 * construit un arbre compresse avec perte sur le taux a partir de l image
	 * this
	...
	/**
	 * fonction recursive de construction descendante des noeuds compresse avec
	 * perte de 1-taux
	 * @param p, noeud courant qui est le pere des prochains
	...
	/**
	 * Enregistre l'image dans un fichier dont le nom est filename et de type 2
	 * ou 5 (P2/P5)
	...
	public void sauvImage(String filename, int type){
		...
		if(type == 2){
			...
			if(type == 5){
				...
				Application.textArea.append("Type incorrect : 2 ou 5"
		...
		if(type == 2){
			...
			if(type == 5){
				...
				for(int i = 0 ; i < getTaille() ; i++){
					for(int j = 0 ; j < getTaille() ; j++){
						((FichierDestinationBinaire) f)
								.ecrireSymboleBinaire(getMat().get(i, j));
					}
				}
			}
		f.fermer();
	 ...
	 */
	public void setMat(Matrice pmat){
		this.mat = pmat;
	}
