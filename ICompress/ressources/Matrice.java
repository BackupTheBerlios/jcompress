...
public class Matrice {
	...
	//coordonnées du prochain symbole à inserer de haut en bas et de gauche a
	// droite
	...
	public void ajoutSymbole(Symbole s){
		...
		if(jCourant + 1 < mat.length){
			...
			if(iCourant + 1 < mat.length){
				//nouvelle ligne
				jCourant = 0;
				iCourant++;
			}
			else{ //System.out.println("matrice pleine");
				iCourant = 0;
				jCourant = 0;
			}
	}
	...
	/**
	 * retourne une sous matrice de mat , id 1 2 3 4
	...
	public Matrice sousMatrice(int id){
		...
		if(getTaille() != 0 && id > 0 && id < 5){
			...
			case 4 :
				...
				break;
			default :
				System.out.println("erreur identification sous matrice");
				break;
			}
		...
		for(int i = 0 ; i < getTaille() ; i++){
			for(int j = 0 ; j < getTaille() ; j++){
				...
				if(somme.containsKey(tmpK))
					tmpV = new Integer(
							(((Integer) somme.get(tmpK)).intValue() + 1));
				else
					tmpV = new Integer(1);
				somme.put(tmpK, tmpV);
			}
	...
	/**
	 * Agrandi la matrice a la taille voulu.
	 * @param pTaille Taille souhaité pour la matrice.
	 * @return Matrice à la taille voulu.
	 */
	public Matrice agrandiMatrice(int pTaille){
		Matrice ma = new Matrice(pTaille);
		if(getTaille() == 1){
			ma = dupliqueMatrice(pTaille);
		}
		else{
			Matrice matNo = sousMatrice(1);
			matNo = matNo.agrandiMatrice(pTaille / 2);

			Matrice matNe = sousMatrice(2);
			matNe = matNe.agrandiMatrice(pTaille / 2);

			Matrice matSo = sousMatrice(3);
			matSo = matSo.agrandiMatrice(pTaille / 2);

			Matrice matSe = sousMatrice(4);
			matSe = matSe.agrandiMatrice(pTaille / 2);

			ma.ajoutMatrice(matNo, NORD_OUEST);
			ma.ajoutMatrice(matNe, NORD_EST);
			ma.ajoutMatrice(matSo, SUD_OUEST);
			ma.ajoutMatrice(matSe, SUD_EST);
		}
	...
	/**
	 * Duplique la matrice jusqu'a la taille voulu.
	 * @param pTaille Taille souhaite.
	 * @return Matrice à la taille voulu.
	 */
	public Matrice dupliqueMatrice(int pTaille){
		Matrice ma = this;
		while(pTaille > ma.getTaille()){
			Matrice temp = new Matrice(ma.getTaille() * 2);
			temp.ajoutMatrice(ma, Matrice.NORD_OUEST);
			temp.ajoutMatrice(ma, Matrice.NORD_EST);
			temp.ajoutMatrice(ma, Matrice.SUD_OUEST);
			temp.ajoutMatrice(ma, Matrice.SUD_EST);
			ma = temp;
		}
		return ma;
	}
